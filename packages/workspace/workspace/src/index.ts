...
declare module '@deepseek-ai/cordis' {
  interface Events {
    /**
     * One stopped persisted session was permanently removed.
     * @mode emit
     * @param sessionId - Removed session id.
     */
...
/** A delete request named a session still owned by a live Agent/session. */
export class WorkspaceActiveSessionError extends Error {
  constructor(readonly sessionId: SessionId) {
    super(`session "${sessionId}" is active and cannot be deleted`)
...
export class WorkspaceRegistry extends Service {
  ...
  /**
   * Permanently delete one persisted session, then remove every workspace
   * membership and archive entry. A durable pending marker lets startup finish
   * the registry cleanup if the two stores are interrupted between writes.
   * @param sessionId - Persisted, non-live session id to remove.
   * @returns A promise that settles after persistence and registry cleanup commit.
   */
  deleteSession(sessionId: SessionId): Promise<void> {
    return this.enqueueOperation(async () => {
      if (this.ctx.get('sessions')?.get(sessionId) !== undefined) {
        throw new WorkspaceActiveSessionError(sessionId)
      }
      const state = this.requireState()
      ...
      try {
        ...
        try {
          ...
          throw new AggregateError(
            [error, rollbackError],
            `session '${sessionId}' deletion failed and its pending marker could not be rolled back`,
          )
        }
        if (error instanceof SessionPersistenceActiveError) {
          throw new WorkspaceActiveSessionError(sessionId)
        }
      ...
      await this.removeSessionReferences(sessionId)
      const nextState: WorkspaceDomainState = {
        initialized: state.initialized,
        workspaceIds: state.workspaceIds,
        archivedSessionIds: state.archivedSessionIds.filter(id => id !== sessionId),
      }
      // If this final registry write fails, the marker remains durable and
      // startup recovery will repeat the idempotent cleanup.
  ...
  private async recoverPendingMutation(): Promise<void> {
    ...
    if (pending.operation === 'delete-session') {
      try {
        await this.ctx.sessionPersistence.delete(pending.sessionId)
      } catch (error: unknown) {
        if (error instanceof SessionPersistenceActiveError) throw error
        if (!(error instanceof SessionPersistenceNotFoundError)) throw error
      }
      await this.removeSessionReferences(pending.sessionId)
      const current = this.requireState()
      await this.setState({
        initialized: current.initialized,
        workspaceIds: current.workspaceIds,
        archivedSessionIds: current.archivedSessionIds.filter(id => id !== pending.sessionId),
      })
      return
    }
  ...
  /** Remove one session id from every durable workspace record. */
  private async removeSessionReferences(sessionId: SessionId): Promise<void> {
    if (this.entities.size > 0) {
      for (const entity of this.entities.values()) await entity.detachSession(sessionId)
      return
    }
    // Startup recovery runs before entities are rebuilt, so update the table
    // directly in that phase.
