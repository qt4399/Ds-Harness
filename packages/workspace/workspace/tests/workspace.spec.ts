...
async function harness(options: HarnessOptions = {}) {
  ...
  const deleteSession = vi.fn(async (id: SessionId) => {
    if (!listed.some(candidate => candidate.id === id)) throw new SessionPersistenceNotFoundError(id)
    listed = listed.filter(candidate => candidate.id !== id)
  })
...
describe('session deletion', () => {
  it('deletes persistence first, then every membership and archive reference', async () => {
    const dir = await makeDir('delete-session')
    const result = await harness({ sessions: [header('kept', dir, 100), header('gone', dir, 200)] })
    const workspace = result.registry.list()[0]!
    await result.registry.archiveSession(SessionId('gone'))
    const deleted: SessionId[] = []
    result.ctx.on('workspace/session-deleted', (id) => { deleted.push(id) })

    await result.registry.deleteSession(SessionId('gone'))

    expect(result.deleteSession).toHaveBeenCalledWith(SessionId('gone'))
  ...
  it('rejects live and unknown sessions without changing durable state', async () => {
    const dir = await makeDir('delete-session-refusals')
    const result = await harness({
      sessions: [header('stored', dir, 100)],
      liveSessions: [header('live', dir, 200)],
    })
    const before = structuredClone(storedState(result.pool))
    ...
    expect(storedState(result.pool)).toEqual(before)
  })

  it('recovers a persisted deletion marker after the session log is already absent', async () => {
    const dir = await makeDir('delete-session-recovery')
    const workspaceId = WorkspaceId('00000000-0000-4000-8000-0000000000dd')
    const pool = storedPool(
      [[workspaceId, record(dir, ['kept', 'doomed'])]],
      {
        initialized: true,
        workspaceIds: [workspaceId],
        archivedSessionIds: [SessionId('doomed')],
        pendingMutation: { operation: 'delete-session', sessionId: SessionId('doomed') },
      },
    )
    ...
    expect(storedRecord(pool, workspaceId).sessionIds).toEqual(['kept'])
    expect(storedState(pool).archivedSessionIds).toEqual([])
    expect(storedState(pool).pendingMutation).toBeUndefined()
  })

  it('recovers registry cleanup after persistence deletion commits first', async () => {
    const dir = await makeDir('delete-session-partial-failure')
    const workspaceId = WorkspaceId('00000000-0000-4000-8000-0000000000de')
    const pool = storedPool(
      [[workspaceId, record(dir, ['kept', 'doomed'])]],
      {
        initialized: true,
    ...
    const first = await harness({
      pool,
      sessions: [header('kept', dir, 100), header('doomed', dir, 200)],
      backend: selectiveFailureBackend(pool, { putAt: 1 }),
    })

    await expect(first.registry.deleteSession(SessionId('doomed')))
      .rejects.toThrow(/selected bootstrap put failure/)
    expect(first.deleteSession).toHaveBeenCalledWith(SessionId('doomed'))
    expect(storedState(pool).pendingMutation).toEqual({
      operation: 'delete-session', sessionId: SessionId('doomed'),
    })
    expect(storedRecord(pool, workspaceId).sessionIds).toEqual(['kept', 'doomed'])
    await first.fiber.dispose()

    const recovered = await harness({ pool, sessions: [] })
    expect(storedRecord(pool, workspaceId).sessionIds).toEqual(['kept'])
    expect(recovered.registry.archivedSessionIds).toEqual([])
    expect(storedState(pool).pendingMutation).toBeUndefined()
  })
})
