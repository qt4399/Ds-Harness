...
- `ctx.workspaceRegistry.deleteSession(id)`：永久删除一个非实时的持久会话，从每个 Workspace 记账和归档集合中移除其 id，并在所有存储提交后发出 `workspace/session-deleted`。实时 id 以 `WorkspaceActiveSessionError` 拒绝；不存在的 id 以 `WorkspaceUnknownSessionError` 拒绝。项目目录与 agent 创建的文件保持不变（见[决策](../../../.agents/notes/implemented/feature/2026-08-15-web-session-deletion.md)）。
...
Workspace 创建／删除和 Session 删除操作会在各自拥有的存储可能分叉之前，先持久化明确的待处理变更标记。启动时只补全该标记所指明的变更，随后清除标记；Session 恢复会将已不存在的日志视为已提交的持久删除，并完成注册表清理。没有标记的顺序／表不一致仍属于来源不明的损坏，并会明确报错。删除后重新注册同一路径会生成新的 Workspace id，且不会自动重新接纳保留下来的会话。
...
- 破坏性文件夹移除仍未提供。删除 Workspace 注册记录绝不会删除 Session，删除 Session 也绝不会删除项目文件（参见[决策记录](../../../.agents/notes/implemented/feature/2026-07-27-workspace-registration-deletion.md)）。
