...
- `ctx.workspaceRegistry.deleteSession(id)` — permanently deletes a non-live persisted session, removes its id from every Workspace account and the archive set, and emits `workspace/session-deleted` after all stores commit. Live ids reject with `WorkspaceActiveSessionError`; absent ids reject with `WorkspaceUnknownSessionError`. Project directories and agent-created files remain untouched ([decision](../../../.agents/notes/implemented/feature/2026-08-15-web-session-deletion.md)).
...
Workspace create/delete and Session deletion persist an explicit pending-mutation marker before their owned stores can diverge. Startup completes only the marked mutation, then clears the marker; Session recovery treats an already-absent log as a committed persistence delete and finishes registry cleanup. An unmarked order/table mismatch remains unexplained corruption and fails loud. Deleting and re-registering the same path creates a fresh Workspace id and does not automatically re-adopt the retained Sessions.
...
- Destructive folder removal remains absent. Workspace registration deletion never deletes Sessions, and Session deletion never deletes project files ([decision](../../../.agents/notes/implemented/feature/2026-07-27-workspace-registration-deletion.md)).
