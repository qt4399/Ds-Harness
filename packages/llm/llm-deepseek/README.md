...
- Some OpenAI-compatible gateways repeat an empty function name on argument-only tool-call deltas; the translator preserves the latest non-empty name instead of allowing that transport quirk to erase the tool identity.
