...
describe('translate: tool calls', () => {
  ...
  it('preserves a non-empty name when a later delta repeats an empty name', async () => {
    const chunks = await collect(translate(feed(
      firstChunk,
      { choices: [{ delta: { tool_calls: [{ index: 0, id: 'call_bash', type: 'function', function: { name: 'bash', arguments: '' } }] } }] },
      { choices: [{ delta: { tool_calls: [{ index: 0, function: { name: '', arguments: '{"command":"pwd"}' } }] } }] },
    ...
    expect(chunks.filter(chunk => chunk.type === 'tool-call-delta')).toEqual([
      ...
      { type: 'tool-call-delta', index: 0, id: 'call_bash', name: 'bash', argumentsDelta: '{"command":"pwd"}' },
    ])
