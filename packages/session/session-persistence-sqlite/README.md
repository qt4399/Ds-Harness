...
- **Delete = a transaction.** `delete()` removes one `sessions` row in a transaction; the foreign key cascades all of its `events` rows. An absent row rolls back and reports `SessionPersistenceNotFoundError`.
