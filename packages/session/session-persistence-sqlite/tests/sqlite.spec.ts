...
describe('SqliteSessionPersistence: durability and crash semantics', () => {
  it('atomically deletes a session row and its cascaded events', async () => {
    ...
    const db = openDatabase(path, 'wal')
    expect((db.prepare('SELECT COUNT(*) AS count FROM events WHERE session_id = ?').get(m.id) as { count: number }).count).toBe(0)
    db.close()
    await dispose()
  })
