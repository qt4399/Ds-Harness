...
export { SessionPersistenceActiveError, SessionPersistenceNotFoundError } from './coordinator.ts'
...
export abstract class SessionPersistence extends Service {
  ...
  /**
   * Permanently remove one persisted session and only artifacts owned by this
   * persistence provider. Live sessions are rejected by the shared coordinator;
   * absent ids reject with {@link SessionPersistenceNotFoundError}.
   * @param id - Persisted session id to remove.
   * @returns A promise that settles after the provider-owned data is removed.
   */
