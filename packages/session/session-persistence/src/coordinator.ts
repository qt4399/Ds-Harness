...
/** The requested session is still owned by a live in-memory Session. */
export class SessionPersistenceActiveError extends Error {
  constructor(readonly sessionId: SessionId) {
    super(`session "${sessionId}" is live and cannot be deleted`)
    this.name = 'SessionPersistenceActiveError'
  }
}

/** No durable artifact exists for the requested session id. */
export class SessionPersistenceNotFoundError extends Error {
  constructor(readonly sessionId: SessionId) {
    super(`session "${sessionId}" was not found in persistence`)
...
export interface PersistenceBackend<TornMarker = unknown> {
  ...
  /**
   * Permanently remove one provider-owned persisted session.
...
export class PersistenceCoordinator<TornMarker = unknown> {
  ...
  /**
   * Permanently remove a persisted session after all earlier writes and
   * retirement work for this id have quiesced. The coordinator is the live
   * ownership boundary; providers only remove their own durable artifacts.
   * @param id - Persisted session id to remove.
   * @returns A promise that settles after deletion or rejects for a live or absent id.
   */
  delete(id: SessionId): Promise<void> {
    const retired = this.waitForRetirement(id)
    return retired.then(() => this.serialize(id, async () => {
      if (this.ctx.sessions.get(id) !== undefined) throw new SessionPersistenceActiveError(id)
