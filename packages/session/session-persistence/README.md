...
| `delete(id): Promise<void>` | Permanently remove only the provider-owned persisted session data. A live id rejects with `SessionPersistenceActiveError`; an absent durable id rejects with `SessionPersistenceNotFoundError`. |
