...
describe('JsonlSessionPersistence: durability and crash semantics', () => {
  ...
  it('deletes only the selected persisted session directory and reports absence', async () => {
    const cwd = await freshRoot()
    const sentinel = join(cwd, 'agent-output.txt')
    await writeFile(sentinel, 'keep me')
    const m = meta('../delete-me', cwd)
    const kept = meta('keep-me', cwd)
    await ctx.sessionPersistence.create(m)
    await ctx.sessionPersistence.append(m.id, oneTurnLog())
    await ctx.sessionPersistence.create(kept)
    await ctx.sessionPersistence.append(kept.id, oneTurnLog())
    ...
    expect((await stat(sessionDir(root, kept.cwd, kept.id))).isDirectory()).toBe(true)
    expect((await ctx.sessionPersistence.list()).map(header => header.id)).toEqual([kept.id])
    await expect(ctx.sessionPersistence.delete(m.id)).rejects.toBeInstanceOf(SessionPersistenceNotFoundError)
  })

  it('refuses to delete a live session without touching its log', async () => {
    const session = ctx.sessions.create(SessionId('delete-live'))
    appendClosedTurn(session)
    await ctx.sessions.flush(session)
    const path = rawLogPath(root, session.header.cwd, session.id)

    await expect(ctx.sessionPersistence.delete(session.id)).rejects.toBeInstanceOf(SessionPersistenceActiveError)
