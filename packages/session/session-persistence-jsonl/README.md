...
- `delete(id)` resolves the stored log by encoded id, validates its header identity, and removes only that session-owned directory. It never removes the source project directory named by `cwd`, sibling session directories, or files created by the agent.
