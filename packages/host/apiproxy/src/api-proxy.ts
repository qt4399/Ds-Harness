...
import {
  ...
  WorkspaceActiveSessionError, WorkspaceMoveInvalidError, WorkspaceOrderInvalidError,
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  /** Agent lifecycles created by this proxy; only their handles authorize teardown. */
  ...
  return {
    ...
    workspace: {
      ...
      async delete(request) {
        ...
        const brandedId = brandWorkspaceId(workspaceId)
        const operation = workspaceCreationChain.then(async () => {
          const workspace = ctx.workspaceRegistry.get(brandedId)
          if (workspace === undefined) return false
          const sessionIds = [...workspace.sessionIds]
          for (const sessionId of sessionIds) {
            await sessionCreations.get(sessionId)
            const handle = ownedSessionHandles.get(sessionId)
            if (handle !== undefined) {
              await handle.dispose()
              if (ownedSessionHandles.get(sessionId) === handle) ownedSessionHandles.delete(sessionId)
            }
            await ctx.workspaceRegistry.deleteSession(sessionId)
          }
          return ctx.workspaceRegistry.delete(brandedId)
        })
        workspaceCreationChain = operation.then(() => undefined, () => undefined)
        try {
