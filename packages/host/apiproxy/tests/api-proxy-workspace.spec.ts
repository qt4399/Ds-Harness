...
async function harness(
  ...
  extras: {
    ...
    canOpenPath?: () => boolean
    persistedSessions?: SessionHeader[]
  } = {},
) {
  ...
  let persistedSessions = structuredClone(extras.persistedSessions ?? [])
  const deletePersisted = vi.fn(async (id: SessionId) => {
    if (!persistedSessions.some(header => header.id === id)) {
      throw new SessionPersistenceNotFoundError(id)
    }
    persistedSessions = persistedSessions.filter(header => header.id !== id)
  })
  ctx.provide('sessionPersistence', {
    list: () => Promise.resolve(structuredClone(persistedSessions)),
  ...
  return { api, ctx, storageDomain, root, deletePersisted, persistedSessions: () => persistedSessions }
}
...
describe('Host Workspace increments', () => {
  ...
  it('deletes one stopped persisted session and maps live or absent ids', async () => {
    const root = realpathSync.native(mkdtempSync(join(tmpdir(), 'dsh-apiproxy-session-delete-')))
    const cwd = stageDir(root, 'delete-home')
    ...
    const result = await harness(root, undefined, { persistedSessions: [header] })
    await result.ctx.workspaceRegistry.archiveSession(sessionId)
    const abort = new AbortController()
    const stream = result.api.events.host(request({}), abort.signal)[Symbol.asyncIterator]()
    const deleted = (async () => {
      for (;;) {
        const candidate = await nextHostFrame(stream)
        if (candidate.payload.type === 'host/session-deleted') return candidate
      }
    })()

    expect(expectOk(await result.api.workspace.deleteSession(request({ sessionId })))).toEqual({ deleted: true })
    expect(await deleted).toMatchObject({
      payload: { type: 'host/session-deleted', sessionId },
    })
    expect(result.deletePersisted).toHaveBeenCalledWith(sessionId)
    expect(result.persistedSessions()).toEqual([])
    const listed = expectOk(await result.api.workspace.list(request({})))
    expect(listed.items[0]?.sessionIds).toEqual([])
    expect(listed.archivedSessionIds).toEqual([])

    const liveId = SessionId('session-live-delete')
    expectOk(await result.api.sessions.create(request({
      workspaceId: listed.items[0]!.workspaceId,
      sessionId: liveId,
    })))
    expect((await result.api.workspace.deleteSession(request({ sessionId: liveId }))).result)
      .toMatchObject({ ok: false, error: { code: 'session-active', details: { sessionId: liveId } } })
    expect((await result.api.workspace.deleteSession(request({ sessionId: SessionId('session-ghost') }))).result)
      .toMatchObject({ ok: false, error: { code: 'session-not-found', details: { sessionId: 'session-ghost' } } })
    abort.abort()
  })
})
