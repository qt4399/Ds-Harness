...
describe('WorkspaceBrowser', () => {
  ...
  it('requires confirmation before permanently deleting an idle non-current session', async () => {
    ...
    expect(screen.getByText(/永久删除会话“delete-me”的记录/)).toBeTruthy()
  ...
  it('blocks duplicate session deletion and keeps failures open for retry or cancellation', async () => {
    let rejectDelete!: (reason: Error) => void
    const pending = new Promise<never>((_resolve, reject) => { rejectDelete = reject })
    const deleteSession = vi.fn()
      .mockReturnValueOnce(pending)
      .mockRejectedValueOnce(new Error('session-active'))
    mount({
      useSessions: hook(sessionState([summary('delete-me', 1)])),
      useWorkspaces: hook(workspaceState([workspace('alpha', ['delete-me'])])),
      deleteSession,
    ...
    rejectDelete(new Error('disk unavailable'))
    await waitFor(() => { expect(screen.getByRole('alert').textContent).toBe('disk unavailable') })
  ...
  it('does not offer deletion for the current or running session', () => {
    const state = sessionState([
      summary('current', 2),
      summary('running', 1, { running: true }),
    ], { current: sid('current') })
    mount({
      useSessions: hook(state),
      useWorkspaces: hook(workspaceState([workspace('alpha', ['current', 'running'])])),
    })
    fireEvent.click(screen.getByRole('button', { name: '会话“current”的操作' }))
    expect(screen.queryByRole('menuitem', { name: '删除会话' })).toBeNull()
    fireEvent.keyDown(document, { key: 'Escape' })
    fireEvent.click(screen.getByRole('button', { name: '会话“running”的操作' }))
    expect(screen.queryByRole('menuitem', { name: '删除会话' })).toBeNull()
  })
