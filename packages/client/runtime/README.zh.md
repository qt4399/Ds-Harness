...
`WorkspaceRuntime.deleteSession(sessionId)` 在一元响应成功时移除永久删除的行，并清空指向它的 selection；`host/session-deleted` 是用于跨标签页同步的幂等回声。该帧不同于 `host/session-removed`：后者只表示实时 Activation 已卸载，因此会保留持久 subagent 行。
