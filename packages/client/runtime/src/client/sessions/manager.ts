...
export class SessionManager {
  ...
  handleHostEnvelope(envelope: RpcRequest<HostFrame>): void {
    ...
    switch (frame.type) {
      ...
      case 'host/session-removed': {
        this.removeSession(frame.sessionId, false)
        return
      }
      case 'host/session-deleted': {
        this.removeSession(frame.sessionId, true)
        return
      }
  ...
  /**
   * Apply a confirmed removal immediately; a later Host echo is idempotent.
   * @param sessionId - Removed session id.
   */
  removePersisted(sessionId: SessionId): void {
    this.removeSession(sessionId, true)
  }

  /** Shared local/Host removal projection; permanent deletion also removes durable subagent rows. */
  private removeSession(sessionId: SessionId, permanent: boolean): void {
    ...
    const durableSubagent = !permanent
    ...
    // A removed session's buffered frames and waits must not replay on a later instantiation.
    this.pendingBuffers.delete(sessionId)
    this.pendingInteractions.delete(sessionId)
    // Owner disposal already dropped jobs on the mux stream, which has no order
    // relative to this Host frame; clear them here whichever arrives first.
    this.jobsBySession.delete(sessionId)
    if (!durableSubagent) this.projectionStores.delete(sessionId)
    // A catalog pull started before removal can return parentAvailable:true.
    // Replay false over that response and request a trailing refresh.
