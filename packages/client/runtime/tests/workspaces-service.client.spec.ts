...
describe('WorkspaceRuntime', () => {
  ...
  it('removes a deleted session on the unary response and keeps failures unchanged', async () => {
    const ctx = new Context()
    const api = new FakeApiClient()
    const sessions = new SessionRuntime(ctx, api, fakeRemote())
    const workspaces = new WorkspaceRuntime(ctx, api, sessions)
    ...
    expect(sessions.list.getSnapshot().ids).toEqual(['s-open'])
    expect(sessions.list.getSnapshot().current).toBe('s-open')

    api.onWorkspaceDeleteSession = () => Promise.resolve(err({
      code: 'session-active', message: 'still live', details: { sessionId: sid('s-open') },
    }))
    await expect(workspaces.deleteSession(sid('s-open'))).rejects.toThrow(/session-active/)
    expect(sessions.list.getSnapshot().ids).toEqual(['s-open'])
    expect(sessions.list.getSnapshot().current).toBe('s-open')

    api.onWorkspaceDeleteSession = () => Promise.resolve(ok({ deleted: true as const }))
    await workspaces.deleteSession(sid('s-open'))
    expect(sessions.list.getSnapshot().ids).toEqual([])
    expect(sessions.list.getSnapshot().current).toBeUndefined()
    sessions.handleHostEnvelope({
      rpcId: 'delete-echo' as never,
      payload: { type: 'host/session-deleted', sessionId: sid('s-open') },
    })
    expect(sessions.list.getSnapshot().ids).toEqual([])
  })
