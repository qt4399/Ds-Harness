...
describe('subagent catalogs', () => {
  ...
  it('distinguishes a detached subagent activation from permanent deletion', async () => {
    const api = new FakeApiClient()
    const manager = new SessionManager(api, fakeRemote())
    manager.handleHostEnvelope({
    ...
    expect(manager.getListSnapshot().items.map(item => item.sessionId)).not.toContain(S2)
  })
})
