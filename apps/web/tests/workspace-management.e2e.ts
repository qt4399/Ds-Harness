...
// across reload), and permanent session deletion. Zero model calls:
...
const DELETE_SEED_ID = 'workspace-management-delete-web-e2e'
...
describe('web e2e: workspace management (create / rename / flat view / hover affordances)', () => {
  ...
  it('permanently deletes a stopped session while preserving project files', async () => {
    ...
    const header = (await scaffold.ctx.sessionPersistence.list())
      .find(candidate => candidate.id === DELETE_SEED_ID)
    if (header === undefined) throw new Error('delete seed did not reach persistence')
    const location = scaffold.ctx.sessionPersistence.locate(header)
    if (location === undefined) throw new Error('JSONL persistence did not expose the delete seed path')
    ...
    const sessionRow = await seededSessionRow()
    const rowTitle = await sessionRow.locator('[class*="title"]').innerText()
    const actionName = `Session actions for ${rowTitle}`

    await clickHoverAction(sessionRow, actionName)
    await page.getByRole('menuitem', { name: 'Delete session' }).click()
    const dialog = page.getByRole('dialog', { name: 'Delete session' })
    await dialog.waitFor({ timeout: 10_000 })
    const copy = await dialog.textContent()
    expect(copy).toContain(rowTitle)
    expect(copy).toContain('permanently deletes the record')
    expect(copy).toContain('Project files and files created by the agent are kept')
    await dialog.getByRole('button', { name: 'Cancel' }).click()
    await dialog.waitFor({ state: 'hidden', timeout: 5_000 })
    await stat(location.path)
    expect((await scaffold.ctx.sessionPersistence.list()).map(candidate => candidate.id))
      .toContain(SessionId(DELETE_SEED_ID))

    await clickHoverAction(sessionRow, actionName)
    await page.getByRole('menuitem', { name: 'Delete session' }).click()
    await page.getByRole('dialog', { name: 'Delete session' })
